...
class Marzban:
    ...
    def __init__(self, username: str, password: str, domain: str, port: int = 443, ssl: str = "https") -> None:
    ...
    async def access_panel(self, username: str = None, password: str = None) -> dict :
        """
        ...
        try:
            ...
            url = f'{self.ssl}://{self.domain}:{self.port}/api/admin/token'
            data = {
                'username': username,
                'password': password
            }
    ...
    async def get_users(self, username: str = None, password: str = None, user_status: str = None, access_panel: dict = None) -> dict:
        """
        ...
        try:
            ...
            user_status = (('?status=' + user_status.lower()) if user_status.lower() in ['on_hold', 'disabled', 'active', 'expired', 'limited'] else '' ) if user_status else ''
            url = f'{self.ssl}://{self.domain}:{self.port}/api/users{user_status}'
            headers = access_panel or await self.access_panel(username=username, password=password)
            async with aiohttp.ClientSession() as session:
    ...
    async def delete_user(self, username: str, access_panel: dict = None) -> bool:
        """
        ...
        """
        try:
            url = f'{self.ssl}://{self.domain}:{self.port}/api/user/{username}'
            headers = access_panel or await self.access_panel()
            async with aiohttp.ClientSession() as session:
    ...
    async def users_statistics(self, username: str = None, password: str = None, users_list: dict = None, prefix: str = '', access_panel: dict = None) -> str:
        """
        ...
        try:
            ...
            password = password or self.password
            headers = access_panel or await self.access_panel(username=username, password=password)
            users_list = users_list or await self.get_users(username=username, password=password, access_panel=headers)
            ...
            all_traffic_remaining = round((all_traffic_limit - all_traffic_used), 3)
            ...
            return f'\nadmin username: {username}\n' \
                ...
                f'all traffic remaining: {all_traffic_remaining} GB\n' \
                f'all time traffic used: {all_time_traffic_used} GB\n'
        except aiohttp.ClientError as e:
            logging.error(f'❌\tError occurred admin traffic: {e}')
            return False

    async def calculation(self, calculation_type: str, data_limit: int, date_limit: int, traffic: int = 0, days: int = 0, coefficient_data: float = 1.0) -> tuple:
        """
        ...
        try:
            if calculation_type == '+':
                ...
                raise ValueError("❌\tInvalid calculation type. Use '+' or '-'")
            return data_limit, date_limit
        except Exception as e:
            logging.error(f"❌\tError occurred during calculation: {e}")
            return False

    async def modify_user(self, modify_type: str, data: bool = 0, date: bool = 0, coefficient_data: float = 1.0, prefix: str = '', username: str = None, password: str = None, access_panel: dict = None) -> str:
        """
        ...
        try:
            ...
            password = password or self.password
            headers = access_panel or await self.access_panel(username=username, password=password)
            users_list = await self.get_users(username=username, password=password, user_status='active', access_panel=headers)
            if modify_type in ['+', '-']:
                for user in users_list:
                    ...
                    if username.startswith(prefix):
                        ...
                        if data_limit is not None and date_limit is not None and data_used is not None:
                            new_data_limit, new_date_limit = await self.calculation(calculation_type=modify_type, data_limit=data_limit, date_limit=date_limit, traffic=data, days=date, coefficient_data=coefficient_data)
                            url = f'{self.ssl}://{self.domain}:{self.port}/api/user/{username}'
                            user_details = {'data_limit': new_data_limit, 'expire': new_date_limit}
                            async with aiohttp.ClientSession() as session:
                                async with session.put(url, json=user_details, headers=headers) as response:
                                    response.raise_for_status()
    ...
    async def users_deleter(self, username: str = None, password: str = None, users_status: str = None, prefix: str = '', last_online: int = 0, access_panel: dict = None) -> str:
        """
        ...
        try:
            ...
            users_list = await self.get_users(username=username, password=password, user_status=users_status, access_panel=headers)
            ...
            for user in users_list:
                ...
                if user_name.startswith(prefix):
                    if user['online_at']:
                        user_last_online = await self.convert_to_secend(user['online_at'])
                        count_all += 1
                        if user_last_online and (user_last_online > (last_online * 86400)):
                            user_delete = await self.delete_user(username=user_name, access_panel=headers)
                            if user_delete:
            ...
            logging.info(f'⚪\tThe process is done!\tdelete {count_delete} user of {count_all} user in {elapsed_time} secend!')
        except aiohttp.ClientError as e:
            logging.error(f'❌\tError occurred users_deleter : {e}')
            return False

    async def convert_to_secend(self, time_str: str) -> int:
        """
        ...
        try :
            ...
            logging.error(f'❌\tError occurred convert to secend : {e}')
            return False

    async def change_status(self, get_user: str = 'active', new_status: str = 'disabled', prefix: str = '', username: str = None, password: str = None, access_panel: dict = None) -> str:
        """
        ...
        try:
            ...
            for user in users_list:
                ...
                if user_name.startswith(prefix):
                    count += 1
                    url = f'{self.ssl}://{self.domain}:{self.port}/api/user/{user_name}'
            ...
            end_time = time.time()
            elapsed_time = end_time - start_time
            logging.info(f'⚪\tThe process is done!\t{count} user is updated in {elapsed_time} secend!')
        except aiohttp.ClientError as e:
...
async def main():
    ...
    SSL = "https" if input('do you have ssl? (y/n) ') == 'y' else "http"
    ...
    if await panel.access_panel() :
        while True :
            ...
            if option == 1 :
                ...
                if option == 1 :
                    print('please wait...')
                    print(await panel.users_statistics())
                elif option == 2 :
                    ...
                    if len(username.split(' ')) == 1 and len(password.split(' ')) == 1 :
                        print('please wait...')
                        print(await panel.users_statistics(username=username, password=password))
                    else :
                    ...
                    if len(prefix.split(' ')) == 1 :
                        print('please wait...')
                        print(await panel.users_statistics(prefix=prefix))
                    else :
                        logging.error('❌\tnot currect, please try again.')
                else :
                    logging.error('❌\tnot currect, please try again.')
                ...
                if option == 1 :
                    print('please wait...')
                    await panel.modify_user(modify_type=modify_type ,data=number_data, coefficient_data=coefficient_data)
                elif option == 2 :
                    username = input('Please enter admin username: ')
                    password = input('Please enter admin password: ')
                    if len(username.split(' ')) == 1 and len(password.split(' ')) == 1 :
                    ...
                    if len(prefix.split(' ')) == 1 :
                        print('please wait...')
                        await panel.modify_user(modify_type=modify_type, data=number_data, coefficient_data=coefficient_data, prefix=prefix)
                    else :
                        logging.error('❌\tnot currect, please try again.')
                else :
                    logging.error('❌\tnot currect, please try again.')
                ...
                if option == 1 :
                    print('please wait...')
                    await panel.modify_user(modify_type=modify_type ,date=date_value)
                elif option == 2 :
                    username = input('Please enter admin username: ')
                    password = input('Please enter admin password: ')
                    if len(username.split(' ')) == 1 and len(password.split(' ')) == 1 :
                        await panel.modify_user(modify_type=modify_type, date=date_value, username=username, password=password)
                    else :
                    ...
                    if len(prefix.split(' ')) == 1 :
                        print('please wait...')
                        await panel.modify_user(modify_type=modify_type, date=date_value, prefix=prefix)
                    else :
                ...
                if option == 1 :
                    print('please wait...')
                    await panel.users_deleter(users_status=users_status, last_online=last_online_time)
                elif option == 2 :
                    username = input('Please enter admin username: ')
                    password = input('Please enter admin password: ')
                    if len(username.split(' ')) == 1 and len(password.split(' ')) == 1 :
                        await panel.users_deleter(users_status=users_status, last_online=last_online_time, username=username, password=password)
                    else :
                        logging.error('❌\tnot currect admin info, please try again.')
                elif option == 3 :
                    prefix = input('Please enter a prefix: ')
                    if len(prefix.split(' ')) == 1 :
                        print('please wait...')
                        await panel.users_deleter(users_status=users_status, last_online=last_online_time, prefix=prefix)
                    else :
                        logging.error('❌\tnot currect prefix, please try again.')
                else :
                    logging.error('❌\tnot currect option, please try again.')
                ...
                if option == 1 :
                    ...
                    password = input('Please enter admin password: ')
                    if len(username.split(' ')) == 1 and len(password.split(' ')) == 1 :
                        await panel.change_status(get_user=users_list_get, new_status=user_new_status, username=username, password=password)
                    else :
                        logging.error('❌\tnot currect admin info, please try again.')
                    ...
                    if len(prefix.split(' ')) == 1 :
                        print('please wait...')
                        await panel.change_status(get_user=users_list_get, new_status=user_new_status, prefix=prefix)
                    else :
                        logging.error('❌\tnot currect prefix, please try again.')
                else :
                    logging.error('❌\tnot currect option, please try again.')
